...
export const sendTelegramMessage = async (text: string): Promise<boolean> => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Telegram API ошибка: ${response.status} - ${errorData}`);
    }
...
💰 <b>Сумма</b>: ${paymentObject.amount?.value ?? "—"} ${
...
📩 <b>Письмо с материалами курса</b>: ${
    emailStatus.success
      ? "✅ Отправлено"
...
export const sendPaymentCanceledNotification = async (
  paymentObject: Payment
) => {
...
👤 Инициатор: ${
    paymentObject.cancellation_details?.party === "merchant"
      ? "Продавец (мы)"
      : paymentObject.cancellation_details?.party === "yoo_money"
      ? "ЮKassa"
      : paymentObject.cancellation_details?.party === "payment_network"
      ? "Платежная сеть (эмитент, сторонний сервис)"
...
💰 <b>Сумма возврата</b>: ${refundObject.amount?.value ?? "—"} ${
