...
router.post("/payment", async (req: Request, res: Response) => {
  try {
    ...
    logger.info('Платеж создан', { paymentId: payment.id, email: req.body.email });
    res.json({ payment });
  } catch (error) {
...
router.post("/payment/notifications", async (req: any, res: any) => {
  logger.debug('Получено уведомление о платеже', { event: req.body.event, type: req.body.type });
  ...
  if (isRefundSucceededNotification(req.body)) {
    if (!validateRefundWebhook(req.body)) {
      logger.error("Невалидная структура webhook для возврата", { body: req.body });
      return res
        .status(400)
    ...
    logger.info("Успешный возврат", { refundId: refundObject.id, paymentId: refundObject.payment_id });

    try {
      await sendRefundSucceededNotification(refundObject);
    } catch (error: any) {
      logger.error("Ошибка отправки уведомления о возврате", { error: error.message, refundId: refundObject.id });
    }
  ...
  if (!validatePaymentWebhook(req.body)) {
    logger.error("Невалидная структура webhook для платежа", { body: req.body });
  ...
  if (isPaymentSucceededNotification(req.body)) {
    ...
    try {
      ...
      try {
        emailStatus = await sendCourseEmail(req.body.object.metadata.email);
      } catch (emailError) {
        const errorInstance = emailError as EmailResult;
        emailStatus = { success: false, message: errorInstance.message || "Ошибка отправки письма" };
      }
    ...
    logger.info("Платёж ожидает подтверждения", { paymentId: req.body.object.id });

    try {
      await sendPaymentWaitingNotification(req.body.object);
    } catch (error: any) {
      logger.error("Ошибка отправки уведомления о платеже ожидающем подтверждения", { 
        error: error.message, 
        paymentId: req.body.object.id 
      });
    }
